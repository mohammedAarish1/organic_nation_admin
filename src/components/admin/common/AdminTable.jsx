...
const AdminProductForm = lazy(
  () => import("../module/admin-products/AdminProductForm"),
);
const AdminBannerForm = lazy(
  () => import("../module/admin-banners/AdminBannerForm"),
);
...
const AdminTable = ({ title, headers, data }) => {
  ...
  const updateStatus = (value) => {
    ...
    let additionalData = {}; // for updating order status as completed
    if (status === "completed") {
      additionalData = { deliveryDate: value };
    }
    ...
    const payload = {
      ...
      status,
      additionalData,
      value: status === "dispatched" ? value : "", // for updating order status as dispatched
    };
    ...
    dispatch(updateCurrentStatus(payload)).then((res) => {
  ...
  const handleDelete = () => {
    ...
    dispatch(deleteDocumentFromDatabase({ collection: type, id })).then(() => {
  ...
  return (
    <div className="  p-4 w-full text-sm font-sans">
      <div className="flex justify-between items-center mb-4 ">
        ...
        {title === "Products" ? (
          <Suspense fallback={<Loader height="100px" />}>
            <AdminProductForm type="add" product={null} />
          </Suspense>
        ) : (
          <button
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full shadow-md rounded-lg table-auto">
          <thead className="bg-[radial-gradient(circle_at_top_left,_rgba(59,130,246,0.25),_transparent_40%),radial-gradient(circle_at_center,_rgba(168,85,247,0.20),_transparent_35%),radial-gradient(circle_at_bottom_right,_rgba(16,185,129,0.25),_transparent_40%),linear-gradient(to_bottom_right,#020617,#0f172a,#111827)] text-white">
            <tr>
              <th className="p-3">
      ...
      <Suspense fallback={<Loader height="50px" />}>
        <Alert
          ...
          hideAlert={hideAlert}
          handleAction={
            curStatusAndId?.status === "" ? handleDelete : updateStatus
          }
          curOrderStatus={curStatusAndId?.status}
        />
      </Suspense>
    </div>
  );
